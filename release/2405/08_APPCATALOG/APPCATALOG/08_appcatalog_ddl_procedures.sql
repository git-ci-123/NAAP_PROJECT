...
--changeset Vijaysree.S:APPCATALOG_DDL_08 splitStatements:false
...
--changeset Vijaysree.S:APPCATALOG_DDL_08_02 splitStatements:false
--preconditions onFail:HALT onError:HALT
...
--changeset Vijaysree.S:APPCATALOG_DDL_08_03 splitStatements:false
--preconditions onFail:HALT onError:HALT
...
--changeset Vijaysree.S:APPCATALOG_DDL_08_04 splitStatements:false
--preconditions onFail:HALT onError:HALT
...
WHERE
    rule_id IN (
        SELECT
            prs.rule_id
        FROM
            business_entity              be, parameter_rule_specification prs
        WHERE
        ...
        WHERE
            rule_id IN (
                SELECT
                    r1.rule_id
                FROM
                    business_entity              be, parameter_rule_specification prs, rule                         r, rule                         r1
                WHERE
                    be.entity_type IN ( 'Market Offering', 'Service', 'Element', 'Feature' )
...
WHERE
    rule_id IN (
        SELECT
            r.rule_id
        FROM
            business_entity              be, parameter_rule_specification prs, rule                         r
        WHERE
            be.entity_type IN ( 'Market Offering', 'Service', 'Element', 'Feature' )
            AND r.rule_id = prs.rule_id
            AND be.bid = prs.parent_id
...
--changeset Vijaysree.S:APPCATALOG_DDL_08_05 splitStatements:false
--preconditions onFail:HALT onError:HALT

CREATE OR REPLACE EDITIONABLE PROCEDURE PURGE_TRACE 
as 
begin
delete from metadata_delete_trace where TRUNC(METADATA_DELETE_EXECUTION_TIME) < sysdate -3 ;
delete from metadata_export_trace where TRUNC(METADATA_EXPORT_EXECUTION_TIME) < sysdate -3 ;
delete from metadata_script_export where TRUNC(CREATED_DATE) < sysdate -3 ;
commit;
end  purge_trace;
